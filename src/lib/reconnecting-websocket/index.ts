 ...
 */
const isWebSocket = (w: unknown) =>
  typeof w !== "undefined" && !!w && (w as { CLOSING: 2 }).CLOSING === 2;
...
export default class ReconnectingWebSocket {
  ...
  private _uptimeTimeout: unknown;
  private _connectTimeout: unknown;
  ...
  /**
   * Closes the WebSocket connection or connection attempt, if unknown. If the connection is already
  ...
  private _getNextUrl(urlProvider: UrlProvider): Promise<string> {
    ...
    if (typeof urlProvider === "function") {
      ...
      // @ts-expect-error: library issue redundant check
      if (url.then) {
        return url;
      }
    }
  ...
  private _connect() {
    ...
    const {
      ...
      WebSocket: WsConstructor = getGlobalWebSocket(),
    } = this._options;
      ...
      .then((url) => {
        ...
        this._ws = this._protocols
          ? // @ts-expect-error: lib issue
            new WsConstructor(url, this._protocols)
          : // @ts-expect-error: lib issue
            new WsConstructor(url);
        if (this._ws) {
  ...
  private _handleOpen = (event: Event) => {
    ...
    clearTimeout(this._connectTimeout as number | undefined);
  ...
  private _clearTimeouts() {
    clearTimeout(this._connectTimeout as number | undefined);
    clearTimeout(this._uptimeTimeout as number | undefined);
  }
}
